...
export default function CategoriesPage() {
  ...
  const uniqueCategories = Array.from(new Set(products.map(product => product.tipo)));
  return (
      <>
    <Header />
    <div className="min-h-screen flex flex-col bg-gradient-to-b from-indigo-900 via-purple-800 to-pink-900 relative overflow-hidden p-6 md:p-8 lg:p-12 mt-30 md:mt-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {uniqueCategories.map((tipo, index) => (
          <Link 
            key={index} 
            href={`/search?tipo=${tipo}`}
          ...
          >
            <h2 className="text-xl font-bold text-purple-900 group-hover:text-indigo-700 transition-colors duration-200">{tipo}</h2>
          </Link>
        ))}
      </div>
    </div>
